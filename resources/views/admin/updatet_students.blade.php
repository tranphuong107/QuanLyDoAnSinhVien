...
">
        ...
        <div class="sidebar">
            ...
            <ul style="list-style: none;">
                <li data-href="{{ URL::to('/student-list') }}" class="actives">
                    <a class="text-light">
                        <i class="fa-solid fa-folder-open" ></i><span>Quản lý sinh viên</span>
                    </a>
                </li>
            </ul>
            <ul style="list-style: none;">
                <li data-href="{{ URL::to('/quanlydt') }}">
                    <a href="{{ URL::to('/quanlydt') }}" style="color:#b4bcc8">
        ...
        <div class="main">
            ...
            <div class="p-3 pt-0">
                <div class="container bg-light p-0" style="max-height:650px;border-radius:5px;box-shadow: 0px 0.1875rem 0.4375rem rgb(0 0 0 / 25%);">
                    <a class="btn_back" style="position: absolute;cursor: pointer;">
                        <i class="bi bi-arrow-left fs-3 ms-2 text-secondary" onclick="popupUpdate()"></i>
                    </a>
                    <h5 class="text-dark mt-2" style="text-align:center">THÔNG TIN SINH VIÊN</h5>
                    ...
                    @endif
                    <form action="{{ URL::to('xuly_capnhat', ['ID' => $data->ID]) }}" method="post">
                        @csrf
                        <div class="row p-0 m-2 gap-5">
                            <div class="col-md-4 col-sm-4 col-xs-12">
                                ...
                                <i class="bi bi-camera-fill text-secondary"
                                    style="position: absolute;top:25rem;left:13rem;cursor: pointer;font-size:35px;color:#f7f7f7;"></i>
                                <input type="file" name="image" accept="image/*" onchange="showFile(event)"
                                    style="position: absolute;top:26rem;left:2rem;cursor: pointer;opacity:0">
                            </div>
                            ...
                            <div class="col-md-2 col-sm-4 col-xs-12 mt-3">
                                ...
                                <div class="">
                                    <div class=" ">
                                        ...
                                        <input type="text" class="form-control" id="floatingInput"
                                            value="{{ $data['PhoneNumber'] }}" name="phone">

                                    </div>
                                </div>
                                <div class="">
                                    <div class=" ">
                                        <label for="floatingInput">Lớp</label>
                                        <select name="lop" class="form-select"
                                            aria-label="Default select example">
                                            @foreach ($data->student as $student)
                                                <option value="{{ $student->class->ID }}">
                                                    {{ $student->class->ClassName }}</option>
                                            @endforeach
                                            @foreach (DB::table('classes')->pluck('ClassName', 'ID') as $id => $class)
                                ...
                                <div class="">
                                    ...
                                    <select name="major_name" class="form-select"
                                        aria-label="Default select example">
                                        @foreach ($data->student as $student)
                                            <option value="{{ $student->class->major->ID }}">
                                                {{ $student->class->major->MajorName }}
                                            </option>
                                        @endforeach
                                        @foreach (DB::table('majors')->pluck('MajorName', 'ID') as $id => $majorname)
                                ...
                                <div class="">
                                    <div class="">
                                        ...
                                        <input type="date" class="form-control" id="floatingInput"
                                            value="{{ $data['DateOfBirth'] }}" name="birthday">
                                    </div>
                                </div>
                                <div class="">
                                    ...
                                    <select name="nation" class="form-select"
                                        aria-label="Default select example">
                                        <option value="Mường" {{ $data['Nation'] == 'Mường' ? 'selected' : '' }}>Mường</option>
                                        <option value="Dao" {{ $data['Nation'] == 'Dao' ? 'selected' : '' }}>Dao</option>
                                        <option value="Thái" {{ $data['Nation'] == 'Thái' ? 'selected' : '' }}>Thái</option>
                                        <option value="Mông" {{ $data['Nation'] == 'Mông' ? 'selected' : '' }}>Mông</option>
                                        <option value="Kinh" {{ $data['Nation'] == 'Kinh' ? 'selected' : '' }}>Kinh</option>
                                    </select>
                                </div>
                                <div class="">
                                    <div class="">
                                        ...
                                        <input type="text" class="form-control" id="floatingInput"
                                            value="{{ $data->student->pluck('StudentCode')->implode(',') }}"
                                            name="masv">
                                    </div>
                                </div>
                                <div class="">
                                    <label for="">Quê quán</label>
                                    <select name="hometown" class="form-select"
                                        aria-label="Default select example">
                                        <option selected>{{ $data['Hometown'] }}</option>
                        ...
                        <button class="btn btn-secondary mb-2 ps-5 pe-5 mb-3 mt-3"
                            style="text-align:center;position:relative;left:45%">Lưu</button>
        ...
        <div class="popupUpdate" id="popupUpdate">
            <div class="ps-5 pt-1 pb-1 pe-5">
                <h5>Thông tin sinh viên vừa chỉnh sửa sẽ không được lưu. Bạn có chắc muốn thoát?</h5>
                <a class="btn btn-secondary mt-3" href="{{ URL::to('/student-list') }}">Thoát</a>
                <p class="close fs-5" onclick="CloseDelete()">x</p>
            </div>
        </div>
        <!-- popup -->
